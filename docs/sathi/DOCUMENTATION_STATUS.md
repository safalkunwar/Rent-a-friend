...
| Messaging metadata inventory | [MESSAGING_METADATA_RESULT.md](MESSAGING_METADATA_RESULT.md) | Complete same-snapshot: 36 conv + 306 msg, all valid parents |
...
| Booking lock containment (local) | [BOOKING_LOCK_CONTAINMENT_GATE.md](BOOKING_LOCK_CONTAINMENT_GATE.md) | Local static contract validated (6/6); emulator pending Java 21+ |
| Staff RBAC containment (local) | [STAFF_RBAC_CONTAINMENT_GATE.md](STAFF_RBAC_CONTAINMENT_GATE.md) | Analytics read scoped to isResourceAdmin; 8/8 static tests; broader RBAC open |
...
| P0-02 staff authority | Initial operator bootstrap and assignment containment released; analytics read scoped; broader RBAC in progress | Live root management and lower-role denial checks passed; six legacy generic roles and 65 remaining isAdmin() resource grants remain unscoped |
| P0-03/P0-04 booking/payment/lock authority | Payment-document write containment deployed; booking-lock containment candidate local; broader work open | Live customer/finance/root payment-write denials passed. Booking lock containment is a local artifact (6/6 static checks) and has NOT been deployed or emulator-verified (Java 17 environment; requires Java 21+). |
...
The combined payment/staff-assignment release is now active as source hash `764eb7a390c58ee077a38fe51798ddc994ac5d8db12c556e6bb41db68c08f402`. Do not repeat it, deploy root Firebase configuration, or use its pre-release baseline for a new patch. Next: remaining legacy staff classification/resource-level RBAC (analytics read scoped to isResourceAdmin; 65 remaining isAdmin() sites open) and booking/lock compatibility against this new active source. The booking-lock containment candidate (hash `3976a456...`) and staff-RBAC containment candidate (hash `9f8d3b0e...`) are ready for emulator validation once Java 21+ is available; Event policy and physical-device/PWA acceptance remain open; test/build counts do not close those gates.
