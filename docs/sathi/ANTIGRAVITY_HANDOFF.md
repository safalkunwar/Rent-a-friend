...
Version 1.1, 2026-09-10. Architecture package complete; Phase 0 evidence collection finished and awaits owner review. Production safety gate failed on reproduced authorization/compatibility issues. This file is the compact entry point, not evidence that proposed features exist.
...
Source baseline inspected: `5ecc696`; browser observation was public guest only. Existing media release and comment repair have dated evidence, not blanket production certification. Production Firestore rules differ from root and ops candidates; root contains unrelated permission changes and contradictory legacy branches. Never deploy it wholesale. Production Storage is scoped; local KYC branches are not proof of deployed access. Legacy Functions include destructive/obsolete handlers; do not bulk activate. Prior billing notes conflict with later media deployment evidence: confirm current billing and explicit authority before new Functions deployment. App Check/PWA upgrade and device acceptance remain unresolved. See [20_TARGET_ARCHITECTURE.md](20_TARGET_ARCHITECTURE.md).

## Phases and next task

Latest correction: [MESSAGING_CONTAINMENT_CORRECTION.md](MESSAGING_CONTAINMENT_CORRECTION.md). Owner approved local messaging-only corrections. The candidate is messaging/favorites-only; the former combined staff/payments artifact is frozen separately without policy changes. Fresh checks:46 corrected-candidate passes. Staff draft remains11 passes/nine denial failures. No deployment or application code changes. The legacy unread-reset writer in `MessagesTab.tsx` was fixed to update only `unreadCount` and `updatedAt`; candidate rules use `affectedKeys().hasOnly(['lastMessage','unreadCount','updatedAt'])`. Do not automatically deploy or advance to staff/payments. The earlier [independent review](CONTAINMENT_REVIEW_2026-09-10.md) remains historical evidence, not the current artifact status.
