...
Uploads fail explicitly by default. `VITE_ENABLE_STORAGE_UPLOADS=true` is an operator deployment opt-in only after owner-confirmed billing, tested/deployed rules, CORS for authenticated reviewer downloads and old-media inventory. This flag does not provide billing, bypass rules or verify security. Never enable it merely to hide an upload error.

Canonical objects: avatars/UID/random-id, posts/UID/random-id, stories/UID/random-id; public JPEG/PNG/WebP <=10 MiB. KYC: kyc/UID/random-id, JPEG/PNG/PDF <=5 MiB, reviewer-only reads. Metadata binds ownerUid/category; overwrite denied. KYC Firestore fields retain the existing documentFileUrl name for compatibility but now hold a private path, not a bearer URL. Public photo and identity document state are separate. Reviewer preview fetches authenticated bytes, not a public token URL. Signature sniffing is client-side defense-in-depth, not server malware scanning.
