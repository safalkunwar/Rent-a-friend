import type { Companion, ExperienceStory, Activity, Event } from '../types';

export const COMPANIONS: Companion[] = [
  {
    id: 'c1',
    name: 'Safal Kunwar',
    age: 24,
    gender: 'Male',
    bio: 'Passionate about trekking, mountain biking, and exploring the hidden gems of Pokhara.',
    hourlyRate: 1500,
    rating: 5.0,
    reviewsCount: 156,
    isVerified: true,
    location: 'Pokhara',
    languages: ['English', 'Nepali'],
    interests: ['Trekking', 'Swimming', 'Outdoors', 'Nature'],
    imageUrl: 'https://scontent.fpkr1-1.fna.fbcdn.net/v/t39.30808-6/595309955_1537495447400950_8564751214332148172_n.jpg?stp=cp6_dst-jpg_tt6&cstp=mx2047x2048&ctp=s2047x2048&_nc_cat=111&ccb=1-7&_nc_sid=6ee11a&_nc_ohc=TCK1vGdFKesQ7kNvwF_Nyib&_nc_oc=AdoaUc0g_ShpzbyRzyNX68Ga807gHs0sd0FKYxg0PmWrR2vr38u-NUhjzIlZz_pplSs&_nc_zt=23&_nc_ht=scontent.fpkr1-1.fna&_nc_gid=lM9nFhFMb_KRw4UV9epiPQ&_nc_ss=7b2a8&oh=00_AQCX1zqDZPuWJqLO5hjsuoseeJXUWpTMAq2M-h62CzO6Rw&oe=6A56363B&auto=format&fit=crop',
    images: ['https://scontent.fpkr1-1.fna.fbcdn.net/v/t39.30808-6/595309955_1537495447400950_8564751214332148172_n.jpg?stp=cp6_dst-jpg_tt6&cstp=mx2047x2048&ctp=s2047x2048&_nc_cat=111&ccb=1-7&_nc_sid=6ee11a&_nc_ohc=TCK1vGdFKesQ7kNvwF_Nyib&_nc_oc=AdoaUc0g_ShpzbyRzyNX68Ga807gHs0sd0FKYxg0PmWrR2vr38u-NUhjzIlZz_pplSs&_nc_zt=23&_nc_ht=scontent.fpkr1-1.fna&_nc_gid=lM9nFhFMb_KRw4UV9epiPQ&_nc_ss=7b2a8&oh=00_AQCX1zqDZPuWJqLO5hjsuoseeJXUWpTMAq2M-h62CzO6Rw&oe=6A56363B&auto=format&fit=crop'],
    availableDays: ['Monday', 'Wednesday', 'Friday', 'Saturday'],
    responseRate: 98,
    responseTime: '~1 hour',
    completedBookings: 142,
    trustScore: 4.9,
    coordinates: { latitude: 28.2096, longitude: 83.9856 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c2',
    name: 'Aarav Thapa',
    age: 26,
    gender: 'Male',
    bio: 'Born and raised in Kathmandu. I love showcasing hidden courtyards, local cafes, and rich history.',
    hourlyRate: 1200,
    rating: 4.9,
    reviewsCount: 124,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['English', 'Nepali', 'Hindi'],
    interests: ['History', 'Photography', 'Local Food'],
    imageUrl: 'https://images.unsplash.com/photo-1600486913747-55e5470d6f40?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1600486913747-55e5470d6f40?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Tuesday', 'Thursday', 'Saturday', 'Sunday'],
    responseRate: 95,
    responseTime: '~2 hours',
    completedBookings: 98,
    trustScore: 4.8,
    coordinates: { latitude: 27.7172, longitude: 85.324 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c3',
    name: 'Priya Gurung',
    age: 24,
    gender: 'Female',
    bio: 'Tech enthusiast by day, foodie by night! Let us explore Patans best cafes.',
    hourlyRate: 1350,
    rating: 4.8,
    reviewsCount: 89,
    isVerified: true,
    location: 'Patan',
    languages: ['English', 'Nepali', 'Hindi'],
    interests: ['Coffee', 'Tech', 'Food'],
    imageUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Tuesday', 'Wednesday', 'Friday'],
    responseRate: 92,
    responseTime: '~3 hours',
    completedBookings: 67,
    trustScore: 4.7,
    coordinates: { latitude: 27.6588, longitude: 85.3247 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c4',
    name: 'Rajan Shrestha',
    age: 28,
    gender: 'Male',
    bio: 'Fitness enthusiast and language learner. Need a workout buddy or practice Nepali?',
    hourlyRate: 1600,
    rating: 5.0,
    reviewsCount: 204,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['English', 'Nepali', 'Newari'],
    interests: ['Fitness', 'Language', 'Outdoors'],
    imageUrl: 'https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Wednesday', 'Friday', 'Saturday', 'Sunday'],
    responseRate: 99,
    responseTime: '~30 mins',
    completedBookings: 198,
    trustScore: 4.9,
    coordinates: { latitude: 27.7089, longitude: 85.3067 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c5',
    name: 'Sita Maharjan',
    age: 25,
    gender: 'Female',
    bio: 'Art student with deep love for architecture and pottery. Join me for a guided walking tour.',
    hourlyRate: 1100,
    rating: 4.7,
    reviewsCount: 56,
    isVerified: true,
    location: 'Bhaktapur',
    languages: ['English', 'Nepali', 'Newari'],
    interests: ['Art', 'History', 'Pottery'],
    imageUrl: 'https://images.unsplash.com/photo-1517841905240-472988babdf9?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1517841905240-472988babdf9?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Tuesday', 'Thursday', 'Saturday'],
    responseRate: 88,
    responseTime: '~4 hours',
    completedBookings: 45,
    trustScore: 4.6,
    coordinates: { latitude: 27.671, longitude: 85.4298 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c6',
    name: 'Nima Sherpa',
    age: 27,
    gender: 'Male',
    bio: 'Mountaineer and outdoor enthusiast. Guide you on short hikes or talk about Everest.',
    hourlyRate: 1300,
    rating: 4.9,
    reviewsCount: 112,
    isVerified: true,
    location: 'Nagarkot',
    languages: ['English', 'Nepali', 'Sherpa'],
    interests: ['Hiking', 'Mountains', 'Local Culture'],
    imageUrl: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Friday', 'Saturday', 'Sunday'],
    responseRate: 94,
    responseTime: '~1 hour',
    completedBookings: 89,
    trustScore: 4.8,
    coordinates: { latitude: 27.686, longitude: 85.521 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c7',
    name: 'Anjali Chhetri',
    age: 23,
    gender: 'Female',
    bio: 'I love exploring local markets, fashion, and traditional crafts. Shopping companion?',
    hourlyRate: 1200,
    rating: 4.9,
    reviewsCount: 78,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['Nepali', 'English'],
    interests: ['Shopping', 'Crafts', 'Fashion'],
    imageUrl: 'https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Wednesday', 'Friday', 'Saturday'],
    responseRate: 90,
    responseTime: '~2 hours',
    completedBookings: 62,
    trustScore: 4.7,
    coordinates: { latitude: 27.7126, longitude: 85.3152 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c8',
    name: 'Yuki Tanaka',
    age: 29,
    gender: 'Female',
    bio: 'Expat photographer who fell in love with Nepal. Best sunrise photo spots in Chitwan.',
    hourlyRate: 2000,
    rating: 5.0,
    reviewsCount: 340,
    isVerified: true,
    location: 'Chitwan',
    languages: ['Japanese', 'English', 'Basic Nepali'],
    interests: ['Photography', 'Wildlife', 'Culture'],
    imageUrl: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
    responseRate: 97,
    responseTime: '~45 mins',
    completedBookings: 310,
    trustScore: 4.9,
    coordinates: { latitude: 27.5291, longitude: 84.3542 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c9',
    name: 'Alex Mercer',
    age: 31,
    gender: 'Male',
    bio: 'Digital nomad and avid reader living in Pokhara. Deep conversations by the lake.',
    hourlyRate: 2200,
    rating: 4.8,
    reviewsCount: 45,
    isVerified: true,
    location: 'Pokhara',
    languages: ['English'],
    interests: ['Business', 'Reading', 'Coffee'],
    imageUrl: 'https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1492562080023-ab3db95bfbce?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Thursday', 'Friday', 'Sunday'],
    responseRate: 85,
    responseTime: '~5 hours',
    completedBookings: 38,
    trustScore: 4.6,
    coordinates: { latitude: 28.2096, longitude: 83.9856 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c10',
    name: 'Tenzing Sherpa',
    age: 30,
    gender: 'Male',
    bio: 'Experienced trekking guide from Langtang. I can take you on peaceful hikes and teach basic Sherpa phrases.',
    hourlyRate: 1800,
    rating: 4.9,
    reviewsCount: 201,
    isVerified: true,
    location: 'Langtang',
    languages: ['English', 'Nepali', 'Sherpa', 'Hindi'],
    interests: ['Hiking', 'Mountains', 'Local Culture'],
    imageUrl: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f15?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1506794778202-cad84cf45f15?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Tuesday', 'Wednesday', 'Friday', 'Saturday'],
    responseRate: 96,
    responseTime: '~1 hour',
    completedBookings: 176,
    trustScore: 4.8,
    coordinates: { latitude: 28.2196, longitude: 85.509 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c11',
    name: 'Kritika Joshi',
    age: 26,
    gender: 'Female',
    bio: 'Coffee lover and digital marketer based in Kathmandu. I know the best quiet cafes for work or chat.',
    hourlyRate: 1400,
    rating: 4.8,
    reviewsCount: 132,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['English', 'Nepali'],
    interests: ['Coffee', 'Business', 'Photography'],
    imageUrl: 'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1524504388940-b1c1722653e1?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Tuesday', 'Wednesday', 'Thursday', 'Saturday', 'Sunday'],
    responseRate: 93,
    responseTime: '~2 hours',
    completedBookings: 104,
    trustScore: 4.7,
    coordinates: { latitude: 27.7172, longitude: 85.324 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c12',
    name: 'Bikram Adhikari',
    age: 32,
    gender: 'Male',
    bio: 'Heritage conservationist from Bhaktapur. Join me for historical walks, pottery sessions, and local storytelling.',
    hourlyRate: 1600,
    rating: 5.0,
    reviewsCount: 189,
    isVerified: true,
    location: 'Bhaktapur',
    languages: ['English', 'Nepali', 'Newari'],
    interests: ['History', 'Art', 'Pottery'],
    imageUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Wednesday', 'Friday', 'Saturday'],
    responseRate: 97,
    responseTime: '~45 mins',
    completedBookings: 156,
    trustScore: 4.9,
    coordinates: { latitude: 27.671, longitude: 85.4298 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c13',
    name: 'Sneha Magar',
    age: 27,
    gender: 'Female',
    bio: 'Wildlife volunteer and photographer based in Chitwan. Let us explore jungles, lakes, and local Tharu culture.',
    hourlyRate: 1700,
    rating: 4.9,
    reviewsCount: 145,
    isVerified: true,
    location: 'Chitwan',
    languages: ['English', 'Nepali'],
    interests: ['Wildlife', 'Photography', 'Nature'],
    imageUrl: 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Thursday', 'Friday', 'Saturday', 'Sunday'],
    responseRate: 94,
    responseTime: '~1 hour',
    completedBookings: 118,
    trustScore: 4.8,
    coordinates: { latitude: 27.5291, longitude: 84.3542 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c14',
    name: 'Rikesh Shahi',
    age: 25,
    gender: 'Male',
    bio: 'Language enthusiast and travel blogger. Practice English or Nepali with me while exploring Pokhara streets.',
    hourlyRate: 1200,
    rating: 4.7,
    reviewsCount: 67,
    isVerified: true,
    location: 'Pokhara',
    languages: ['English', 'Nepali', 'Hindi'],
    interests: ['Language Exchange', 'Photography', 'Food'],
    imageUrl: 'https://images.unsplash.com/photo-1504257432389-52343c08ab9c?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1504257432389-52343c08ab9c?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Tuesday', 'Thursday', 'Friday', 'Saturday'],
    responseRate: 91,
    responseTime: '~3 hours',
    completedBookings: 54,
    trustScore: 4.6,
    coordinates: { latitude: 28.2096, longitude: 83.9856 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c15',
    name: 'Sunita Rai',
    age: 29,
    gender: 'Female',
    bio: 'Street food explorer and cooking instructor. Join me for a tasty evening around Kathmandu markets.',
    hourlyRate: 1500,
    rating: 4.9,
    reviewsCount: 178,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['English', 'Nepali'],
    interests: ['Food', 'Local Culture', 'Photography'],
    imageUrl: 'https://images.unsplash.com/photo-1489424731084-269b73c91e86?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1489424731084-269b73c91e86?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Tuesday', 'Wednesday', 'Saturday', 'Sunday'],
    responseRate: 95,
    responseTime: '~1 hour',
    completedBookings: 142,
    trustScore: 4.8,
    coordinates: { latitude: 27.7089, longitude: 85.3067 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c16',
    name: 'Dil Maya Tamang',
    age: 24,
    gender: 'Female',
    bio: 'Cultural dance instructor from Pokhara. I can teach you traditional dances and take you to local performances.',
    hourlyRate: 1100,
    rating: 4.8,
    reviewsCount: 98,
    isVerified: true,
    location: 'Pokhara',
    languages: ['English', 'Nepali', 'Tamang'],
    interests: ['Culture', 'Music', 'Local Culture'],
    imageUrl: 'https://images.unsplash.com/photo-1508214751196-bcfd4ca60f91?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1508214751196-bcfd4ca60f91?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Wednesday', 'Friday', 'Sunday'],
    responseRate: 92,
    responseTime: '~2 hours',
    completedBookings: 74,
    trustScore: 4.7,
    coordinates: { latitude: 28.2096, longitude: 83.9856 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c17',
    name: 'Kiran Basnet',
    age: 28,
    gender: 'Male',
    bio: 'Cycling tour guide and fitness coach around Kathmandu valleys. Bring your helmet and energy!',
    hourlyRate: 1700,
    rating: 4.9,
    reviewsCount: 156,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['English', 'Nepali'],
    interests: ['Cycling', 'Fitness', 'Outdoors'],
    imageUrl: 'https://images.unsplash.com/photo-1568602471122-7832951cc4c5?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1568602471122-7832951cc4c5?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Tuesday', 'Thursday', 'Saturday', 'Sunday'],
    responseRate: 95,
    responseTime: '~1 hour',
    completedBookings: 128,
    trustScore: 4.8,
    coordinates: { latitude: 27.7172, longitude: 85.324 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c18',
    name: 'Meera Sharma',
    age: 26,
    gender: 'Female',
    bio: 'Yoga instructor and meditation guide. Join me for sunrise sessions by Phewa Lake or peaceful garden walks.',
    hourlyRate: 1900,
    rating: 5.0,
    reviewsCount: 210,
    isVerified: true,
    location: 'Pokhara',
    languages: ['English', 'Nepali', 'Hindi'],
    interests: ['Fitness', 'Nature', 'Outdoors'],
    imageUrl: 'https://images.unsplash.com/photo-1531123897727-8f129e1688ce?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1531123897727-8f129e1688ce?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Wednesday', 'Friday', 'Saturday', 'Sunday'],
    responseRate: 98,
    responseTime: '~30 mins',
    completedBookings: 182,
    trustScore: 4.9,
    coordinates: { latitude: 28.2096, longitude: 83.9856 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c19',
    name: 'Arjun Poudel',
    age: 31,
    gender: 'Male',
    bio: 'Volunteer coordinator and community builder. Join local clean-up walks, youth meetups, and social events.',
    hourlyRate: 1000,
    rating: 4.7,
    reviewsCount: 89,
    isVerified: true,
    location: 'Lalitpur',
    languages: ['English', 'Nepali'],
    interests: ['Community Event', 'Volunteer', 'Culture'],
    imageUrl: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Saturday', 'Sunday'],
    responseRate: 90,
    responseTime: '~4 hours',
    completedBookings: 56,
    trustScore: 4.6,
    coordinates: { latitude: 27.6588, longitude: 85.3247 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c20',
    name: 'Pooja Karki',
    age: 25,
    gender: 'Female',
    bio: 'Fashion design student and shopping buddy. Let us explore Kathmandus vintage streets and craft markets.',
    hourlyRate: 1300,
    rating: 4.8,
    reviewsCount: 112,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['English', 'Nepali'],
    interests: ['Fashion', 'Shopping', 'Crafts'],
    imageUrl: 'https://images.unsplash.com/photo-1524503034941-e6b2b1f7a1a8?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1524503034941-e6b2b1f7a1a8?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Tuesday', 'Thursday', 'Friday', 'Saturday'],
    responseRate: 93,
    responseTime: '~2 hours',
    completedBookings: 85,
    trustScore: 4.7,
    coordinates: { latitude: 27.7089, longitude: 85.3067 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c21',
    name: 'Suman Giri',
    age: 33,
    gender: 'Male',
    bio: 'Professional photographer and videographer. Book me for shoots around Kathmandu or Pokhara landscapes.',
    hourlyRate: 2500,
    rating: 4.9,
    reviewsCount: 276,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['English', 'Nepali'],
    interests: ['Photography', 'Music', 'Business'],
    imageUrl: 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'],
    responseRate: 96,
    responseTime: '~1 hour',
    completedBookings: 230,
    trustScore: 4.8,
    coordinates: { latitude: 27.7172, longitude: 85.324 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c22',
    name: 'Ritu Khadka',
    age: 27,
    gender: 'Female',
    bio: 'History researcher and museum guide. I run curated heritage walks around Patan and Bhaktapur.',
    hourlyRate: 1400,
    rating: 4.9,
    reviewsCount: 134,
    isVerified: true,
    location: 'Patan',
    languages: ['English', 'Nepali', 'Newari'],
    interests: ['History', 'Museum', 'Culture'],
    imageUrl: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Tuesday', 'Wednesday', 'Friday', 'Saturday'],
    responseRate: 94,
    responseTime: '~1 hour',
    completedBookings: 109,
    trustScore: 4.8,
    coordinates: { latitude: 27.6588, longitude: 85.3247 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c23',
    name: 'Binod Mahato',
    age: 29,
    gender: 'Male',
    bio: 'Coffee roaster and cafe consultant. Let us taste local beans and learn about Nepali coffee culture.',
    hourlyRate: 1600,
    rating: 4.8,
    reviewsCount: 91,
    isVerified: true,
    location: 'Kathmandu',
    languages: ['English', 'Nepali', 'Hindi'],
    interests: ['Coffee', 'Business', 'Food'],
    imageUrl: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'],
    responseRate: 91,
    responseTime: '~2 hours',
    completedBookings: 73,
    trustScore: 4.7,
    coordinates: { latitude: 27.7172, longitude: 85.324 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c24',
    name: 'Anita Limbu',
    age: 26,
    gender: 'Female',
    bio: 'Language tutor and trek planner. Practice Nepali or plan your next hike with a local expert from Dharan.',
    hourlyRate: 1200,
    rating: 4.7,
    reviewsCount: 64,
    isVerified: true,
    location: 'Dharan',
    languages: ['English', 'Nepali', 'Limbu'],
    interests: ['Language Exchange', 'Hiking', 'Study'],
    imageUrl: 'https://images.unsplash.com/photo-1542596594-649edbc13630?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1542596594-649edbc13630?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Monday', 'Wednesday', 'Friday', 'Saturday'],
    responseRate: 89,
    responseTime: '~3 hours',
    completedBookings: 48,
    trustScore: 4.6,
    coordinates: { latitude: 26.8149, longitude: 87.2833 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'c25',
    name: 'Rohit Gurung',
    age: 30,
    gender: 'Male',
    bio: 'Paragliding instructor and adventure planner. Experience Pokhara from the sky with a certified pilot.',
    hourlyRate: 3000,
    rating: 5.0,
    reviewsCount: 312,
    isVerified: true,
    location: 'Pokhara',
    languages: ['English', 'Nepali'],
    interests: ['Hiking', 'Outdoors', 'Photography'],
    imageUrl: 'https://images.unsplash.com/photo-1504257432389-52343c08ab9c?q=80&w=800&auto=format&fit=crop',
    images: ['https://images.unsplash.com/photo-1504257432389-52343c08ab9c?q=80&w=800&auto=format&fit=crop'],
    availableDays: ['Tuesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'],
    responseRate: 97,
    responseTime: '~45 mins',
    completedBookings: 265,
    trustScore: 4.9,
    coordinates: { latitude: 28.2096, longitude: 83.9856 },
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
];

export const STORIES: ExperienceStory[] = [
  {
    id: 's1',
    companionId: 'c1',
    userId: 'u-demo-1',
    userName: 'Emma',
    companionName: 'Safal Kunwar',
    imageUrl: 'https://scontent.fpkr1-1.fna.fbcdn.net/v/t39.30808-6/595309955_1537495447400950_8564751214332148172_n.jpg?stp=cp6_dst-jpg_tt6&cstp=mx2047x2048&ctp=s2047x2048&_nc_cat=111&ccb=1-7&_nc_sid=6ee11a&_nc_ohc=TCK1vGdFKesQ7kNvwF_Nyib&_nc_oc=AdoaUc0g_ShpzbyRzyNX68Ga807gHs0sd0FKYxg0PmWrR2vr38u-NUhjzIlZz_pplSs&_nc_zt=23&_nc_ht=scontent.fpkr1-1.fna&_nc_gid=lM9nFhFMb_KRw4UV9epiPQ&_nc_ss=7b2a8&oh=00_AQCX1zqDZPuWJqLO5hjsuoseeJXUWpTMAq2M-h62CzO6Rw&oe=6A56363B&auto=format&fit=crop',
    timeAgo: '2h ago',
    caption: 'Amazing momo tasting tour!',
    likes: 124,
    comments: 18,
  },
  {
    id: 's2',
    companionId: 'c2',
    userId: 'u-demo-2',
    userName: 'Raj',
    companionName: 'Aarav Thapa',
    imageUrl: 'https://images.unsplash.com/photo-1510425463958-dcced28da480?q=80&w=800&auto=format&fit=crop',
    timeAgo: '4h ago',
    caption: 'Swayambhunath was breathtaking at sunset.',
    likes: 89,
    comments: 12,
  },
  {
    id: 's3',
    companionId: 'c5',
    userId: 'u-demo-3',
    userName: 'Chloe',
    companionName: 'Sita Maharjan',
    imageUrl: 'https://images.unsplash.com/photo-1522075469751-3a6694fb2f61?q=80&w=800&auto=format&fit=crop',
    timeAgo: '6h ago',
    caption: 'Patan Durbar Square is so rich in history.',
    likes: 56,
    comments: 7,
  },
  {
    id: 's4',
    companionId: 'c4',
    userId: 'u-demo-4',
    userName: 'Liam',
    companionName: 'Nima Sherpa',
    imageUrl: 'https://images.unsplash.com/photo-1511216113906-8f56bb20925c?q=80&w=800&auto=format&fit=crop',
    timeAgo: '1d ago',
    caption: 'A great boat ride on Phewa lake.',
    likes: 210,
    comments: 24,
  },
  {
    id: 's5',
    companionId: 'c8',
    userId: 'u-demo-5',
    userName: 'Sophia',
    companionName: 'Yuki Tanaka',
    imageUrl: 'https://images.unsplash.com/photo-1498654896293-37aacf113fd9?q=80&w=800&auto=format&fit=crop',
    timeAgo: '2d ago',
    caption: 'Coffee tasting and great conversations.',
    likes: 167,
    comments: 15,
  },
];

export const ACTIVITIES: Activity[] = [
  {
    id: 'a1',
    title: 'Local Coffee Chat',
    description: 'Explore hidden cafes with a local companion.',
    duration: '1-2 hours',
    avgPrice: 1500,
    imageUrl: 'https://images.unsplash.com/photo-1497935586351-b67a49e012bf?q=80&w=800&auto=format&fit=crop',
    companionCount: 124,
    category: 'Coffee Buddy',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'a2',
    title: 'Street Food Tour',
    description: 'Taste the best street food in the city.',
    duration: '2-3 hours',
    avgPrice: 2000,
    imageUrl: 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?q=80&w=800&auto=format&fit=crop',
    companionCount: 85,
    category: 'Food Explorer',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
  {
    id: 'a3',
    title: 'City Photography',
    description: 'Capture the city skyline and hidden gems.',
    duration: '2-4 hours',
    avgPrice: 2500,
    imageUrl: 'https://images.unsplash.com/photo-1516862523118-a3724eb136d7?q=80&w=800&auto=format&fit=crop',
    companionCount: 62,
    category: 'Photography Walk',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  },
];

export const EVENTS: Event[] = [
  {
    id: 'e1',
    title: 'Weekend Hiking Group',
    description: 'Group hike around the valley with experienced guides.',
    date: '2026-07-24',
    time: '10:00 AM',
    location: 'National Park Trailhead',
    coordinates: { latitude: 27.7172, longitude: 85.324 },
    spots: 3,
    participants: ['u-demo-1', 'u-demo-2'],
    imageUrl: 'https://images.unsplash.com/photo-1551632811-561732d54612?q=80&w=800&auto=format&fit=crop',
    createdBy: 'u-demo-admin',
    createdAt: new Date().toISOString(),
  },
  {
    id: 'e2',
    title: 'Art Exhibition & Coffee',
    description: 'Visit the downtown gallery and discuss art over coffee.',
    date: '2026-07-25',
    time: '02:00 PM',
    location: 'Downtown Gallery',
    coordinates: { latitude: 27.7089, longitude: 85.3067 },
    spots: 2,
    participants: ['u-demo-3'],
    imageUrl: 'https://images.unsplash.com/photo-1460661419201-fd4cecdf8a8b?q=80&w=800&auto=format&fit=crop',
    createdBy: 'u-demo-admin',
    createdAt: new Date().toISOString(),
  },
  {
    id: 'e3',
    title: 'Language Exchange Meetup',
    description: 'Practice languages with native speakers in a relaxed setting.',
    date: '2026-07-26',
    time: '06:30 PM',
    location: 'Central Cafe',
    coordinates: { latitude: 27.7126, longitude: 85.3152 },
    spots: 5,
    participants: ['u-demo-1', 'u-demo-4', 'u-demo-5'],
    imageUrl: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?q=80&w=800&auto=format&fit=crop',
    createdBy: 'u-demo-admin',
    createdAt: new Date().toISOString(),
  },
  {
    id: 'e4',
    title: 'Photography Walk',
    description: 'Capture the historic district at golden hour.',
    date: '2026-07-28',
    time: '09:00 AM',
    location: 'Historic District',
    coordinates: { latitude: 27.671, longitude: 85.4298 },
    spots: 1,
    participants: [],
    imageUrl: 'https://images.unsplash.com/photo-1476673160081-cf0658b76f3e?q=80&w=800&auto=format&fit=crop',
    createdBy: 'u-demo-admin',
    createdAt: new Date().toISOString(),
  },
];
