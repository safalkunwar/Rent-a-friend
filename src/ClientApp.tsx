            ...
            <div className="px-4 py-1 space-y-4">
              ...
              <div className="space-y-4">
                {fetchedStories.map((post, idx) => {
                  ...
                  return (
                    <div key={`${post.id || 'st'}-${idx}`} className="bg-card border border-border-token-light rounded-3xl overflow-hidden shadow-lg flex flex-col text-left">
                      {/* Top profile header */}
                      <div className="p-3 flex justify-between items-center bg-card">
                        <div className="flex items-center gap-2.5">
                          <SafeImage src={post.userAvatar} className="w-8 h-8 rounded-full object-cover border border-primary-action" alt={post.userName} fallbackType="avatar" textForInitials={post.userName} />
                          <div>
                            <span className="text-xs font-bold text-text-primary block leading-tight">{post.userName}</span>
                            <span className="text-[9px] text-text-secondary flex items-center gap-0.5">
                              <MapPin className="w-2.5 h-2.5 text-primary-action" /> with companion {post.companionName}
                            </span>
                          </div>
                        </div>
                        <span className="text-[10px] bg-primary-action/10 text-primary-action font-extrabold px-2 py-0.5 rounded-full border border-primary-action/20">
                          🌟 SATHI Co-Experience
                        </span>
                      </div>
                      
                      {/* Large Portrait Image - No dark overlay! */}
                      <div className="relative aspect-[4/4.5] w-full bg-surface-elevated">
                        <SafeImage src={post.imageUrl} className="w-full h-full object-cover" alt="Adventure moment" fallbackType="thumbnail" />
                      </div>

                      {/* Clean high-contrast caption area below image */}
                      <div className="px-4 pt-3 pb-1">
                        <p className="text-xs text-text-primary leading-relaxed font-medium">{post.caption}</p>
                      </div>
                      
                      {/* Bottom action panel */}
                      <div className="p-3 flex justify-between items-center border-t border-border-token-light bg-card">
                        <div className="flex items-center gap-4">
                          <button 
                            onClick={() => handleToggleLikeMoment(post.id)}
                            className="flex items-center gap-1.5 text-text-primary active:scale-90 transition-transform"
                          >
                            <Heart className={`w-4 h-4 ${isLiked ? 'text-red-500 fill-current' : 'text-text-secondary hover:text-red-500'}`} />
                          ...
                          <button 
                            ...
                            className="flex items-center gap-1.5 text-text-secondary hover:text-primary-action transition-colors"
                          >
                        ...
                        <span className="text-[9px] text-text-muted font-bold">{post.timeAgo || '2h ago'}</span>
                      </div>
                    </div>
                  );
                })}
