...
export const EsewaVerifyPage: React.FC = () => {
  ...
  useEffect(() => {
    const verify = async () => {
      ...
      try {
        ...
        const result = await paymentService.verifyPayment('esewa', transactionUuid);
        if (result.success) {
          setStatus('success');
          setMessage('eSewa payment verified successfully! Your booking is confirmed.');
        } else {
          setStatus('failed');
          setMessage('eSewa payment verification failed. Please contact support.');
        }
      } catch (err) {
        setStatus('failed');
