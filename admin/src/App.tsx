    ...
    <AdminAuthGuard>
      ...
      <aside className="w-64 bg-background border-r border-border-token hidden md:flex flex-col">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
           ...
           <button onClick={() => setActiveTab('reports')} className={`w-full flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === 'reports' ? 'bg-primary-action/10 text-primary-action' : 'text-text-secondary hover:text-text-primary hover:bg-surface-elevated'}`}>
      ...
      <main className="flex-1 pb-20 md:pb-0 overflow-y-auto">
         ...
         <div className="md:hidden h-14 bg-background border-b border-border-token flex items-center justify-between px-4 sticky top-0 z-50">
           ...
           <div className="md:hidden flex overflow-x-auto bg-background border-b border-border-token hide-scrollbar">
             {['overview', 'users', 'guides', 'companions', 'bookings', 'content', 'moderation', 'security', 'feedback', 'audit', 'reports'].map((tab) => (
               <button
                  key={tab}
