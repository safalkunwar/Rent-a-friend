import { doc, getDoc, getDocs, setDoc, updateDoc, deleteDoc, query, where, orderBy, limit, startAfter, onSnapshot, collection, type Unsubscribe, type Query, type DocumentData, type Firestore } from 'firebase/firestore';
import { db } from '../firebase';

type QueryCondition = {
  field: string;
  operator: '==' | '!=' | '<' | '>' | '<=' | '>=' | 'array-contains' | 'in' | 'array-contains-any';
  value: unknown;
};

export interface QueryOptions {
  where?: QueryCondition[];
  orderByField?: string;
  orderDirection?: 'asc' | 'desc';
  limitCount?: number;
  startAfter?: unknown[];
}

const buildQuery = <T = DocumentData>(collectionName: string, options: QueryOptions = {}): Query<T> => {
  const base = doc(db!, collectionName);
  let q: Query<T> = base as unknown as Query<T>;

  if (options.where) {
    for (const condition of options.where) {
      q = query(q, where(condition.field, condition.operator, condition.value as any));
    }
  }

  if (options.orderByField) {
    q = query(q, orderBy(options.orderByField, options.orderDirection || 'asc'));
  }

  if (options.startAfter && options.startAfter.length > 0 && options.orderByField) {
    q = query(q, startAfter(...options.startAfter));
  }

  if (options.limitCount) {
    q = query(q, limit(options.limitCount));
  }

  return q;
};

const requireDb = (): Firestore => {
  if (!db) throw new Error('[SATHI Admin] Firebase Firestore is not configured.');
  return db;
};

export const firestore = {
  collection: <T = DocumentData>(name: string) => collection(requireDb(), name),

  getDocument: async <T = DocumentData>(path: string): Promise<T | null> => {
    try {
      const snap = await getDoc(doc(requireDb(), path));
      if (!snap.exists()) return null;
      return { id: snap.id, ...snap.data() } as T;
    } catch (error) {
      console.error('[SATHI Admin] Firestore getDocument error:', error);
      return null;
    }
  },

  setDocument: async (path: string, data: Record<string, unknown>, merge = false) => {
    try {
      const ref = doc(requireDb(), path);
      await setDoc(ref, data, { merge });
      return ref.id;
    } catch (error) {
      console.error('[SATHI Admin] Firestore setDocument error:', error);
      throw error;
    }
  },

  updateDocument: async (path: string, data: Record<string, unknown>) => {
    try {
      const ref = doc(requireDb(), path);
      await updateDoc(ref, data);
      return ref.id;
    } catch (error) {
      console.error('[SATHI Admin] Firestore updateDocument error:', error);
      throw error;
    }
  },

  deleteDocument: async (path: string) => {
    try {
      await deleteDoc(doc(requireDb(), path));
    } catch (error) {
      console.error('[SATHI Admin] Firestore deleteDocument error:', error);
    }
  },

  getDocuments: async <T = DocumentData>(collectionName: string, options: QueryOptions = {}): Promise<T[]> => {
    if (!db) return [];
    try {
      const q = buildQuery<T>(collectionName, options);
      const snap = await getDocs(q);
      return snap.docs.map(d => ({ id: d.id, ...d.data() } as T));
    } catch (error) {
      console.error('[SATHI Admin] Firestore getDocuments error:', error);
      return [];
    }
  },

  getDocumentsPaginated: async <T = DocumentData>(collectionName: string, options: QueryOptions = {}): Promise<{ items: T[]; lastVisible?: unknown[]; hasMore: boolean }> => {
    if (!db) return { items: [], hasMore: false };
    try {
      const q = buildQuery<T>(collectionName, options);
      const snap = await getDocs(q);
      const items = snap.docs.map(d => ({ id: d.id, ...d.data() } as T));
      const lastVisible = snap.docs.length > 0 ? snap.docs[snap.docs.length - 1].data() as unknown[] : undefined;
      const hasMore = snap.docs.length >= (options.limitCount || 0);
      return { items, lastVisible, hasMore };
    } catch (error) {
      console.error('[SATHI Admin] Firestore getDocumentsPaginated error:', error);
      return { items: [], hasMore: false };
    }
  },

  subscribe: <T = DocumentData>(collectionName: string, options: QueryOptions = {}, callback: (items: T[]) => void): Unsubscribe => {
    if (!db) {
      callback([]);
      return () => {};
    }
    const q = buildQuery<T>(collectionName, options);
    return onSnapshot(
      q,
      (snap) => {
        const items = snap.docs.map(d => ({ id: d.id, ...d.data() } as T));
        callback(items);
      },
      (error) => {
        console.error('[SATHI Admin] Firestore subscribe error:', error);
      }
    );
  },

  subscribeDocument: <T = DocumentData>(path: string, callback: (item: T | null) => void): Unsubscribe => {
    if (!db) {
      callback(null);
      return () => {};
    }
    return onSnapshot(
      doc(db, path),
      (snap) => {
        if (!snap.exists()) {
          callback(null);
          return;
        }
        callback({ id: snap.id, ...snap.data() } as T);
      },
      (error) => {
        console.error('[SATHI Admin] Firestore subscribeDocument error:', error);
      }
    );
  },
};
